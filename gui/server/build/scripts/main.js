...
var ser = new _crial.Crial();
var streaming = false;
ser.onDataCb(function (data) {
    if (streaming) io.sockets.emit('console.log', data);
});
...
io.on('connection', function (socket) {
    ...
    socket.on('command', function (comando) {
        ...
        switch (comando[0]) {
            case "exit":
                ser.close(function (k) {
                    console.log("serial cerrado");
                    process.exit();
                });
                break;
            case "getAllLog":
                ser.getLoger().getAllLog(function (logData) {
